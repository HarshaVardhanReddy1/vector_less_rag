...
def generate_judge_prompt(
    ...
    return f"""
You are an independent answer quality judge.

Your task is to evaluate a generated answer strictly against the provided source context.
You did NOT generate this answer. Evaluate it critically and objectively.
...
Score each metric as an integer between 0 and 100.

1. accuracy_score
   Is the answer factually correct based ONLY on the source context?

   100 = every claim is directly and correctly supported by the context
    80 = mostly correct, minor imprecision
    60 = partially correct, some unsupported claims
    40 = weak support, significant inaccuracies
     0 = incorrect or contradicts the context

2. groundedness_score
   Does the answer stay within what the source context actually says?

   100 = every sentence can be traced to the context
    70 = mostly grounded, minor additions
    40 = partially grounded, notable hallucinations
     0 = mostly or fully hallucinated

3. relevance_score
   Does the answer actually address the user query?

   100 = directly and completely answers the query
    70 = partially answers the query
    40 = loosely related but misses the point
     0 = irrelevant
...
Choose ONE based on the strength of evidence in the context:

- "high"   → context clearly and completely supports the answer
- "medium" → context partially supports, some ambiguity
- "low"    → context weakly supports or the answer extrapolates
...
Your reasoning must:
1. Identify which parts of the context support or contradict the answer.
2. Call out any claims in the answer not present in the context.
3. Explain how well the answer addresses the query.
4. Be specific — reference actual content, not generic statements.
...
{{
  "reasoning": "<specific explanation referencing both the answer and the context>",
