...
# Retry budget for each batch. Kept low because failed requests still count
# against the free-tier daily quota — one short backoff, then give up.
_ENRICH_MAX_ATTEMPTS        = 2
_ENRICH_BACKOFF_MAX_SECONDS = 30


# JSON-schema structured output: forces the model to return
# {"sections": [{summary, keywords, node_id}, ...]} — guaranteeing valid,
# parseable JSON instead of free-form text. (object root because strict
# structured outputs do not allow a top-level array.)
_ENRICHMENT_RESPONSE_FORMAT = {
    "type": "json_schema",
    "json_schema": {
...
def _enrich_chunk(
    ...
    for attempt in range(1, _ENRICH_MAX_ATTEMPTS + 1):
        try:
            ...
            if attempt == 1:
                kwargs["response_format"] = _ENRICHMENT_RESPONSE_FORMAT
            ...
            if attempt < _ENRICH_MAX_ATTEMPTS:
                time.sleep(min(5 * attempt, _ENRICH_BACKOFF_MAX_SECONDS))
