...
# ---------------------------------------------------------------------------
# Validation helpers (delegating to validation module)
# ---------------------------------------------------------------------------
...
def validate_selection_response(
    ...
    """Validate a single-node selection response."""
    if not isinstance(json_data, dict):
        raise ValidationError("Selection output must be a JSON object.")

    selected_node_id = json_data.get("selected_node_id")
    reason = validate_non_empty_string(json_data.get("reason"), "reason")

    if selected_node_id is None:
        return {"selected_node_id": None, "reason": reason}

    validated_id = validate_node_id(selected_node_id, "selected_node_id")
...
def validate_multi_selection_response(
    ...
    """Validate a multi-node selection response."""
    if not isinstance(json_data, dict):
        raise ValidationError("Selection output must be a JSON object.")
    ...
    if not isinstance(selected_node_ids, list):
        raise ValidationError("Field 'selected_node_ids' must be a JSON array.")

    validated_ids = []
    for nid in selected_node_ids:
        validated_id = validate_node_id(nid, "selected_node_ids item")
        if allowed_node_ids is not None and validated_id not in allowed_node_ids:
            raise ValidationError(
                f"Selected node_id '{validated_id}' is not present in the allowed candidates."
            )
